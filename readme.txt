...
* [ 不具合修正 ] GitHub API への通信に失敗した際、プラグイン詳細画面の表示で PHP8 の警告が発生する不具合を修正
* [ 不具合修正 ] GitHub リリースに zip ファイルが添付されていない場合、更新情報の取得に失敗する不具合を修正
* [ 不具合修正 ] ショートコード [vk-plugin-list] の表示時に、環境によっては致命的エラー（Fatal error）が発生する不具合を修正
* [ セキュリティ修正 ] ショートコード [vk-plugin-list] の説明文出力を WordPress コア準拠の許可タグに制限し安全化
